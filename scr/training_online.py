...
from scr.training import TrainResult, _adam_update, _loss_grad
...
def train_online_hard(
    ...
    for step in range(train_config.steps):
        x, y, desirable, _true_cluster, annotator = dataset.sample(train_config.batch_size)
    ...
    _append_final_eval(theta, world, train_config, eval_steps, eval_quality, log_prefix, "hard_online", seed)
    return OnlineTrainResult(
        train_result=TrainResult("hard_online", seed, np.asarray(eval_steps), np.asarray(eval_quality), None, final_theta=theta.copy()),
...
def train_online_soft(
    ...
    _append_final_eval(theta, world, train_config, eval_steps, eval_quality, log_prefix, "soft_online", seed)
    return OnlineTrainResult(
        train_result=TrainResult("soft_online", seed, np.asarray(eval_steps), np.asarray(eval_quality), None, final_theta=theta.copy()),
        refit_steps=np.asarray(refit_steps),
        nmi=np.asarray(nmi),
        purity=np.asarray(pur),
        coverage=np.asarray(coverage),
